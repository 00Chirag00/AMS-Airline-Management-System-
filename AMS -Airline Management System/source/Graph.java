...
{
	//DATA MEMBERS
	public String [] cty= {"Allahabad","Delhi","Mumbai","Chennai","Bangalore","Pune","Jaipur","Bhiwani","Goa"};
		...
		for(int i=0; i<c ; ++i)
		{
			for(int j=0; j<c; ++j)
			{
				if(i!=j)
				{
					...
					time[i][j].getA().set(Calendar.MONTH,11);
					time[i][j].getA().set(Calendar.DAY_OF_MONTH,20);
					time[i][j].getA().set(Calendar.YEAR,2022);
				}
			}
			...
			for(int i=0;i<count;i++)
			{
				for(int j=0;j<count;j++)
				{
					if(!(i==j))
					{
						...
						d1.set(2022, 11, 20, x, x1, 0) ;
						Calendar d2= Calendar.getInstance();
						d2.set(2022, 11, 20, y, y1, 0);
						Flight f1 = new Flight("GRP49IT"+id,30,cty[i],cty[j],d1,d2);
						temp = new Node(f1);
						gr[i][j].insert(temp);
					}
				}
